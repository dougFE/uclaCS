...
// Then get Arena::display going.  That gives
...
const int MAXCOLS = 25; //25              // max number of columns in the arena
...
private:
    ...
    int    m_row;
    int    m_col;
    int m_lives = 2; // Should start at 2. 1 is slow, 0 is dead
    bool m_turn = true; // Tracks movement turns for rabbits
    // DONE: You'll probably find that a rabbit object needs additional
...
void Rabbit::move()
{
    //cout << "Rabbit move called" << endl;
    if ((m_lives == 1) && (m_turn == false)) { // If poisoned and other turn, do nothing
        //cout << "Skipped turn from poisoning" << endl;
        m_turn = true;
        return;
    }
    // DONE:
    //   Return without moving if the rabbit has eaten one poisoned
    //   carrot and this is a turn it does not move.
    ...
    //cout << "Attempted move in direction " << direction << endl;

    if ((m_lives == 1) && (m_turn == true)) { // If hurt and just moved, set turn false to halt move next turn
        m_turn = false;
    }

    if (m_arena->getCellStatus(m_row, m_col) == HAS_POISON) { // If lands on carrot, take 1 life & dont move next turn
        m_lives--;
        m_turn = false;
        m_arena->setCellStatus(m_row, m_col, EMPTY); // Remove carrot after eating
    }
    return;
}
...
string Player::move(int dir)
{
    ...
    case 1:
        ...
        break;
    case 2:
        cout << "Player moved south." << endl;
        break;
    case 3:
        cout << "Player moved west." << endl;
        break;
    }
    return "Player couldn't move; player stands.";
}
...
Arena::~Arena()
{
    // DONE destructor implementation for deleting player and rabbits
    delete m_player;
...
{
    // DONE: numRabbits implementation
    int count = 0;
    for (int i = 0; i < m_nRabbits; i++) {
...
void Arena::display(string msg) const
{
    ...
    for (r = 1; r <= rows(); r++) {
        for (c = 1; c <= cols(); c++) {
            rabbitNum = 0; // Reset for each new cell
            displayGrid[r - 1][c - 1] = (getCellStatus(r, c) == EMPTY ? '.' : '*');
            // Rabbit coordinates start at 1
            rabbitNum = numberOfRabbitsAt(r, c); // save num of rabbits
            if(rabbitNum == 1){
                displayGrid[r - 1][c - 1] = 'R';
            }
            else if (rabbitNum > 1 && rabbitNum < 9) {
                displayGrid[r - 1][c - 1] = '0' + rabbitNum;
            }
            else if (rabbitNum > 9) {
...
bool Arena::addRabbit(int r, int c)
{
    ...
    Rabbit* tempRabbit = new Rabbit(this, r, c); // Needs pointer to arena as first param
    m_rabbits[m_nRabbits] = tempRabbit;
    m_nRabbits++; // Increment number of rabbits now that it's been successfully added
    return true;
}
...
{
    //cout << "Move rabbits called" << endl;
    // Move all rabbits
    // DONE?:  Move each rabbit.  Mark the player as dead if necessary.
    //        Deallocate any dead dynamically allocated rabbit.

    for (int i = 0; i < m_nRabbits; i++) {
        if (m_rabbits[i] == nullptr) {
            continue; // Skip if rabbit deleted
        }
        //cout << "Working with rabbit number " << i << endl;
        if (m_rabbits[i]->isDead()) {
            m_nRabbits--;
            delete m_rabbits[i]; // todo: Investigate if this causes bugs cause of missing array elements
            m_rabbits[i] = m_rabbits[m_nRabbits];
            m_rabbits[m_nRabbits] = nullptr;
        }
        else
        {
            //cout << "Moving rabbit number " << i << endl;
            m_rabbits[i]->move(); // Move the rabbit
...
bool attemptMove(const Arena& a, int dir, int& r, int& c)
{
    ...
    if (dir % 2 == 0) { // North 0 or south 2
        //cout << "moving n/s" << endl;
        if (r + (dir - 1) < 1 || r + (dir - 1) > a.rows())
            return false;
        r += (dir - 1);
        return true;
    }
    else if (dir % 2 == 1) { // East 1 or west 3
        //cout << "moving w/e" << endl;
        if (c + (-dir + 2) < 1 || c + (-dir + 2) > a.cols())
            return false;
        c += (-dir + 2);
        return true;
    }
...
// Helper function to calculate danger rating of cell
float cellDanger(const Arena& a, int r, int c) {
    float count = 0;
    if (a.numberOfRabbitsAt(r, c) > 0) {
        return 1000000; // If rabbit(s) in cell, return max danger rating
    }

    if (a.getCellStatus(r, c) == HAS_POISON) {
        count += 1; // Add danger for poisoned squares to encourage choosing empty squares
    }
    float distFromCenter = 0.0;
    if (r > a.rows() / 2.0)
        distFromCenter += r - (a.rows() / 2.0);
    if (r < a.rows() / 2.0)
        distFromCenter += (a.rows() / 2.0)-r;
    if (c > a.cols() / 2.0)
        distFromCenter += c - (a.cols() / 2.0);
    if (c < a.cols() / 2.0)
        distFromCenter += (a.cols() / 2.0) - c;
    

    count += distFromCenter * 0.0001; // Encourage moving towards center with slight danger increases 

    // Check surrounding cells and add number of rabbits to danger counter
    if (r - 1 > 0)
        count += 2 * a.numberOfRabbitsAt(r - 1, c);
    if (r + 1 <= a.rows())
        count += 2 * a.numberOfRabbitsAt(r + 1, c);
    if (c - 1 > 0)
        count += 2 * a.numberOfRabbitsAt(r, c - 1);
    if (c + 1 <= a.cols())
        count += 2 * a.numberOfRabbitsAt(r, c + 1);
    return count;
}
...
{
    // DONE:  Implement this function
    float minDanger = cellDanger(a, r, c);
    bestDir = -1;
    if (minDanger < 1 && a.getCellStatus(r, c) != HAS_POISON) { // If on safe tile with no carrot, drop one
        return false;
    }
    for (int i = 0; i < 2; i++){
        if ((r + (2 * i - 1) > 0) && (r + (2 * i - 1) <= a.rows())) { // North and south dirs
            if (cellDanger(a, r + (2 * i - 1), c) < minDanger || (cellDanger(a, r+ (2 * i - 1), c) == minDanger && randInt(0, 1) == 1)) {
                minDanger = cellDanger(a, r + (2 * i - 1), c);
                bestDir = i * 2;
            }
        }
        if ((c + (2 * i - 1) > 0) && (c + (2 * i - 1) <= a.cols())) { // East and west dirs
            if ((cellDanger(a, r, c + (2 * i - 1)) < minDanger) || (cellDanger(a, r, c + (2 * i - 1)) == minDanger && randInt(0,1) == 1)) {
                minDanger = cellDanger(a, r, c + (2 * i - 1));
                bestDir = ((-2) * i)+3; // convert from 1 and 0 to 3 and 1;
            }
        }
    }
